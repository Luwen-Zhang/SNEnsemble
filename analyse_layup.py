...
leaderboard = leaderboard.drop(
    leaderboard[leaderboard["Model"].str.contains("TransformerLayup")].index
).reset_index(drop=True)
improved_measure, ttest_res = trainer.get_modelbase("ThisWorkLayup").improvement(
    leaderboard,
    cv_path=os.path.join(trainer.project_root, "cv"),
    exclude=[x for x in trainer.leaderboard["Model"] if "TransformerLayup" in x],
)
improved_measure.to_csv(os.path.join(trainer.project_root, "improvement.csv"))
method_ranking, detailed = trainer.get_modelbase("ThisWorkLayup").method_ranking(
    improved_measure,
    leaderboard,
    exclude=[x for x in trainer.leaderboard["Model"] if "TransformerLayup" in x],
)
...
# )
trainer.get_modelbase("ThisWorkLayup").plot_compare(
    leaderboard,
    improved_measure,
    ttest_res,
    metric="Testing RMSE",
...
trainer.get_modelbase("ThisWorkLayup").plot_improvement(
    leaderboard,
    improved_measure,
    ttest_res,
