...
class GMMSN(nn.Module):
    def __init__(self, n_clusters: int, n_input: int, layers):
        ...
        self.gmm = GMM(n_clusters=n_clusters, n_input=n_input, clusters=self.sns)
        self.knn = False
        self.k = int(np.sqrt(n_clusters))

    def forward(self, x, s, naive_pred):
        x_cluster = self.gmm(x)
        out = naive_pred.clone()
        if not self.knn or self.n_clusters == 1:
            for i_cluster, cluster in enumerate(self.gmm.clusters):
            ...
            knn = self.gmm.k_nearest_neighbors(k=self.k)
            for i_cluster, cluster in enumerate(self.gmm.clusters):
                idx_in_cluster = torch.where(x_cluster == i_cluster)[0]
                if len(idx_in_cluster) >= 2:
                    preds = [
                        self.gmm.clusters[i_nn_cluster].sn(
                            x[idx_in_cluster],
                            s[idx_in_cluster],
                            naive_pred[idx_in_cluster, :],
                        )
