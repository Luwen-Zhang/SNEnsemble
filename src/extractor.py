...
class Extractor():
    ...
    def read(self):
        ...
        self._df = pd.read_excel(
            self.data_path, engine='openpyxl', sheet_name=None)
        ...
        self._feature_names = self._scatter_feature_names.copy()
        ...
        data = pd.DataFrame(columns=list(assign.columns) + self._feature_names)
        ...
        for row in assign.iterrows():
            ...
            if len(assign.columns) > 1:
                prop_to_assign = [x for x in list(assign.columns)[
                    1:] if pd.notna(row[1][x])]
            else:
                prop_to_assign = []
            ...
            for property in prop_to_assign:
                assigned_property_feature = list(
                    self._df[property].columns)[1:]
                assigned_features += assigned_property_feature

            # Create the dataframe for the scatter
            tmp_df = pd.DataFrame(columns=list(
                row[1].keys()) + list(self._df[scatter_name].columns) + assigned_features)
            ...
            for property in prop_to_assign:
                ...
                assigned_property_index = list(
                    self._df[property]['Property']).index(row[1][property])
                assigned_property_value = self._df[property].loc[assigned_property_index,
                                                                 assigned_property_feature]

                for row_idx in tmp_df.index:
                    tmp_df.loc[row_idx,
                               assigned_property_feature] = assigned_property_value
    ...
    def apply_transform(self, column_from, column_to, transform_func, indexes=[], **kargs):
        '''
    ...
    def fill_na(self, column, criterion, remove_na_axis):
        ...
        train_data = np.array(
            data.loc[train_index, train_features], dtype=np.float32)
        train_label = np.array(column_data[train_index], dtype=np.float32)
        pred_data = np.array(
            data.loc[pred_index, train_features], dtype=np.float32)
        ...
        if criterion == 'RandomForest':
            ...
            print('\tPredictors: ', train_features, '\n\tTraining set',
                  list(train_index), '\n\tPred set', list(pred_index))
            print(
                f'\tR2 score {rf.score(train_data, train_label):.5f}.')
    ...
    def interpolate_from_curves(self, ignore_property=True):
        ...
        def check_available_interp():
            ...
            for idx, relation in enumerate(self._curve_relation):
                if ignore_property:
                    ...
                    curve_prop_fit = self.check_curve_property_fit(
                        self._curves[idx])
                    available_interp[idx, 0] = len(np.intersect1d(
                        self.check_available_transform(
                            relation[0], relation[1])[0],
                        curve_prop_fit
                    ))
                    available_interp[idx, 1] = len(np.intersect1d(
                        self.check_available_transform(
                            relation[1], relation[0])[0],
                        curve_prop_fit
                    ))
        ...
        while np.sum(available_interp) != 0:
            ...
            best_interp = np.where(
                available_interp == np.max(available_interp))
            best_interp_relation = best_interp[0][0]
            best_interp_from = best_interp[1][0]

            self.interpolate_from_curve(
                self._curves[best_interp_relation], best_interp_from, ignore_property=ignore_property)
    ...
    def check_curve_property_fit(self, curve_name):
        ...
        curve_relation = self._curve_relation[self._curves.index(
            curve_name)]
        prop_feature = list(curve_df.columns)
        prop_feature = [
            x for x in prop_feature if x not in curve_relation and x not in self._properties]
...
def create_example(data_path):
    ...
    property_1 = pd.DataFrame({'Property': ['P1', 'P2', 'P3'], 'A': np.random.randn(
        3), 'B': np.random.randn(3)})
    property_2 = pd.DataFrame(
        {'Property': ['P4', 'P5', 'P6'], 'B': np.random.randn(3), 'D': np.random.randn(3)})

    property_1.loc[0, 'B'] = np.nan

    scatter_1 = pd.DataFrame(
        {'D': np.random.randn(5), 'E': np.random.randn(5)})
    scatter_2 = pd.DataFrame(
        {'A': np.random.randn(5), 'C': np.random.randn(5)})
    scatter_3 = pd.DataFrame(
        {'D': np.random.randn(5), 'E': np.random.randn(5)})

    curve_1 = pd.DataFrame(
        {'C': np.linspace(-2, 2, 10), 'D': np.linspace(-2, 2, 10)})
    curve_2 = pd.DataFrame(
        {'A': np.linspace(-2, 2, 10), 'C': np.linspace(-2, 2, 10)})
    curve_3 = pd.DataFrame(
        {'A': np.linspace(-2, 2, 10), 'F': np.linspace(-2, 2, 10)})

    assign = pd.DataFrame({'Scatter': ['Scatter 1', 'Scatter 2', 'Scatter 3', 'Curve 1', 'Curve 2', 'Curve 3'], 'Property 1': [
                          'P1', np.nan, 'P2', 'P1', np.nan, np.nan], 'Property 2': [np.nan, 'P4', np.nan, np.nan, 'P4', np.nan]})
