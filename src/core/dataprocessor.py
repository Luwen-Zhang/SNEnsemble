...
class UnscaledDataRecorder(AbstractProcessor):
    ...
    def fit_transform(self, input_data: pd.DataFrame, trainer: Trainer):
        feature_data, label_data, derived_data = trainer._divide_from_tabular_dataset(input_data)

        trainer.unscaled_feature_data = feature_data
        trainer.unscaled_label_data = label_data
        trainer.unscaled_derived_data = derived_data
...
class MeanImputer(AbstractTransformer):
    ...
    def fit_transform(self, input_data: pd.DataFrame, trainer: Trainer):
        ...
        data.loc[fit_indices, trainer.stacked_feature_names] = imputer.fit_transform(
            data.loc[fit_indices, trainer.stacked_feature_names]).astype(np.float32)
        data.loc[trans_indices, trainer.stacked_feature_names] = imputer.transform(
            data.loc[trans_indices, trainer.stacked_feature_names]).astype(np.float32)
...
class StandardScaler(AbstractTransformer):
    ...
    def fit_transform(self, input_data: pd.DataFrame, trainer: Trainer):
        ...
        fit_indices = np.intersect1d(np.array(list(trainer.train_indices) + list(trainer.val_indices)),
                                     np.array(data.index))
        trans_indices = np.setdiff1d(np.array(data.index), fit_indices)
        data.loc[fit_indices, trainer.stacked_feature_names] = scaler.fit_transform(
            data.loc[fit_indices, trainer.stacked_feature_names]).astype(np.float32)
        data.loc[trans_indices, trainer.stacked_feature_names] = scaler.transform(
            data.loc[trans_indices, trainer.stacked_feature_names]).astype(np.float32)
